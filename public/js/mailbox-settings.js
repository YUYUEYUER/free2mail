...
export function renderForwardBadge(forwardTo) {
  if (!forwardTo) return '';
  return `<span class="badge badge-forward" title="转发到: ${escapeHtml(forwardTo)}">${icon('forward')}</span>`;
}
...
export function renderFavoriteBadge(isFavorite) {
  return isFavorite ? `<span class="badge badge-favorite" title="已收藏">${icon('star')}</span>` : '';
}
...
export function injectDialogStyles() {
  ...
  style.textContent = `
    ...
    .modal-overlay .modal-close:hover {
      color: #1d1d1f;
      border-color: rgba(53, 102, 238, 0.18);
    }
    ...
    .modal-overlay .form-input:focus {
      outline: none;
      border-color: rgba(53, 102, 238, 0.4);
      box-shadow: 0 0 0 4px rgba(53, 102, 238, 0.1);
    }
    .modal-overlay .badge {
      display: inline-flex;
      align-items: center;
    ...
    .modal-overlay .badge-forward {
      background: rgba(53, 102, 238, 0.1);
      color: #3566ee;
    }
    .modal-overlay .badge-favorite {
