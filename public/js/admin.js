...
function renderMailboxPanelEmptyState(user = null) {
  const isSelected = !!user;
  const title = isSelected
    ? `${user.username} 还没有邮箱样本`
    : '等待锁定一个用户面板';
  const description = isSelected
    ? '可以从左侧快速分配邮箱，让该用户的收件样本、登录状态和跳转入口出现在这里。'
    : '先从左侧用户表选中一个账号，这里会切换成对应的研究面板与邮箱关系视图。';
  const stats = isSelected
    ? [
        { label: '用户名', value: user.username },
        { label: '当前绑定', value: `0 / ${user.mailbox_limit || 0}` },
        { label: '建议动作', value: '使用上方分配邮箱' }
      ]
    : [
        { label: '当前状态', value: '未选中用户' },
        { label: '面板用途', value: '邮箱归属 / 快速跳转' },
        { label: '下一步', value: '点击任意用户行' }
      ];

  return `
    <div class="research-panel-empty research-panel-empty--admin">
      <div class="research-panel-empty__eyebrow">
        <span class="research-panel-empty__badge">${icon(isSelected ? 'inbox' : 'boxes')}</span>
        <span>${isSelected ? 'User mailbox panel' : 'Mailbox relation panel'}</span>
      </div>
      <div class="research-panel-empty__body">
        <div class="research-panel-empty__copy">
          <div class="research-panel-empty__title">${escapeText(title)}</div>
          <div class="research-panel-empty__description">${escapeText(description)}</div>
        </div>
        <div class="research-panel-empty__stats">
          ${stats.map((stat) => `
            <div class="research-panel-empty__stat">
              <span class="research-panel-empty__stat-label">${escapeText(stat.label)}</span>
              <span class="research-panel-empty__stat-value">${escapeText(stat.value)}</span>
            </div>
          `).join('')}
        </div>
      </div>
...
function syncMailboxPanelState() {
  const container = els.userMailboxes?.querySelector('.mailbox-list');
  if (!container) return;
  if (els.mailboxesRefresh) {
    els.mailboxesRefresh.style.display = currentViewingUser ? 'inline-flex' : 'none';
  }
  if (!currentViewingUser) {
    container.innerHTML = renderMailboxPanelEmptyState();
  }
}
...
async function loadUserMailboxes() {
  ...
  try {
    ...
    if (container) {
      ...
      `).join('') : renderMailboxPanelEmptyState(currentViewingUser);
...
els.mailboxesRefresh?.addEventListener('click', () => { if (currentViewingUser) loadUserMailboxes(); });
