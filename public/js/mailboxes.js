...
const preferredDefaultView = window.matchMedia?.('(max-width: 900px)').matches ? 'list' : 'grid';
...
const SEARCH_DEBOUNCE_MS = (() => {
  try {
    ...
    return (connection?.saveData || lowCpu || lowMemory) ? 480 : 320;
  } catch (_) {
    return 320;
  }
})();
...
function renderEmptyState() {
  if (!els.empty) return;
  els.empty.innerHTML = renderOverviewEmptyState({
    query: els.q?.value,
    domain: els.domainFilter?.value,
    login: els.loginFilter?.value,
...
function updatePager() {
  ...
  const pager = els.page?.closest('.pager');
  if (pager) pager.style.display = lastCount > 0 ? 'flex' : 'none';
}
...
function bindCardEvents() {
  ...
  els.grid?.querySelectorAll('.mailbox-card[data-action="jump"]').forEach(card => {
    card.onclick = (e) => {
      // 如果点击的是按钮区域，不跳转
      if (e.target.closest('.card-actions') || e.target.closest('[data-action]:not(.mailbox-card)')) return;
...
function openPasswordModal(address, isDefault) {
  ...
  if (isDefault) {
    // 设置新密码
    if (els.passwordModalIcon) setIcon(els.passwordModalIcon, 'lock');
    ...
    // 重置密码
    if (els.passwordModalIcon) setIcon(els.passwordModalIcon, 'key');
...
function openBatchModal(action, title, iconName, message) {
  ...
  if (els.batchModalIcon) setIcon(els.batchModalIcon, iconName);
...
els.q?.addEventListener('input', () => { if (searchTimeout) clearTimeout(searchTimeout); searchTimeout = setTimeout(() => { page = 1; load(); }, SEARCH_DEBOUNCE_MS); });
...
// 批量操作按钮
els.batchAllow?.addEventListener('click', () => openBatchModal('allow', '批量放行登录', 'check', '输入要允许登录的邮箱地址（每行一个或用逗号分隔）：'));
els.batchDeny?.addEventListener('click', () => openBatchModal('deny', '批量禁止登录', 'ban', '输入要禁止登录的邮箱地址（每行一个或用逗号分隔）：'));
els.batchFavorite?.addEventListener('click', () => openBatchModal('favorite', '批量收藏', 'star', '输入要收藏的邮箱地址（每行一个或用逗号分隔）：'));
els.batchUnfavorite?.addEventListener('click', () => openBatchModal('unfavorite', '批量取消收藏', 'star', '输入要取消收藏的邮箱地址（每行一个或用逗号分隔）：'));
els.batchForward?.addEventListener('click', () => openBatchModal('forward', '批量设置转发', 'forward', '输入要设置转发的邮箱地址（每行一个或用逗号分隔）：'));
els.batchClearForward?.addEventListener('click', () => openBatchModal('clear-forward', '批量清除转发', 'ban', '输入要清除转发的邮箱地址（每行一个或用逗号分隔）：'));
