...
(function(){
  try{
    ...
    var icon = function(name){ return '<svg class="ui-icon" aria-hidden="true"><use href="/icons.svg#'+name+'"></use></svg>'; };
    const els = {
    ...
    try{
      var setupMainSwitch = function(){
        ...
        var showGen = function(){
          ...
          try{
            ...
            if (!existingEnter && genCard && actions){
              ...
              existingEnter.innerHTML = '<span class="btn-icon">'+icon('inbox')+'</span><span>进入邮箱</span>';
        ...
        var showHis = function(){
          ...
          if (switchWrap) switchWrap.style.display = '';
          try{ if (typeof window.primeMailboxList === 'function') window.primeMailboxList(); }catch(_){ }
          lastMainView = 'his';
                ...
                if (header){
                  ...
                  if (!existing){
                    ...
                    iconBtn.style.padding = '0';
                     iconBtn.innerHTML = '<span class="btn-icon" style="margin:0">'+icon('refresh')+'</span>';
