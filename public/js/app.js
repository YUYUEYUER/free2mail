      ...
      <div class="research-panel-empty__body">
        ...
        <div class="research-panel-empty__stats">
          ...
          <div class="research-panel-empty__stat">
            ...
            <span class="research-panel-empty__stat-value">${REFRESH_INTERVAL} 秒轮询一次</span>
          </div>
          <div class="research-panel-empty__stat">
...
async function refresh() {
  ...
  try {
    ...
    if (!Array.isArray(emails) || !emails.length) { els.list.innerHTML = renderInboxEmptyPanel(mailbox, isSentViewActive()); if (els.pager) els.pager.style.display = 'none'; return; }
    const isMobile = window.matchMedia?.('(max-width: 900px)').matches;
    els.list.innerHTML = sliceByPage(emails, els).map(e => renderEmailItem(e, isMobile)).join('');
    if (!isSentViewActive() && PERF_MODE.prefetchDetails) prefetchEmails(emails, api);
    markViewLoaded();
  } catch (_) {}
...
// 视图切换
if (els.tabInbox) els.tabInbox.onclick = () => { setView(false); els.tabInbox.classList.add('active'); els.tabSent?.classList.remove('active'); if (els.boxTitle) els.boxTitle.textContent = '收件箱'; if (els.boxIcon) setIcon(els.boxIcon, 'inbox'); resetPager(els); refresh(); };
if (els.tabSent) els.tabSent.onclick = () => { setView(true); els.tabSent.classList.add('active'); els.tabInbox?.classList.remove('active'); if (els.boxTitle) els.boxTitle.textContent = '发件箱'; if (els.boxIcon) setIcon(els.boxIcon, 'send'); resetPager(els); refresh(); };
