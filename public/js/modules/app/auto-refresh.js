...
function canRefresh() {
  if (!isAutoRefreshEnabled || !isPageVisible || !isWindowFocused) {
    return false;
  }
...
export function initVisibilityTracking() {
  if (visibilityTrackingInitialized) return;
  visibilityTrackingInitialized = true;
  try { isWindowFocused = typeof document.hasFocus === 'function' ? document.hasFocus() : true; } catch (_) { isWindowFocused = true; }
  document.addEventListener('visibilitychange', () => {
    isPageVisible = !document.hidden;
    if (isPageVisible && canRefresh()) {
...
/**
 * 设置自动刷新的激活条件
 * @param {Function|null} guard - 返回是否允许刷新的函数
 */
export function setRefreshGuard(guard) {
  refreshGuard = typeof guard === 'function' ? guard : null;
}
