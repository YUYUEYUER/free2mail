      ...
      <div class="item-content">
        ...
        <div class="item-meta">
          ...
          <span class="item-indicators">
            ${isFavorite ? '<span class="indicator favorite" title="已收藏">FAV</span>' : ''}
            ${forwardTo ? `<span class="indicator forward" title="转发至: ${escapeAttr(forwardTo)}">FWD</span>` : ''}
            ${canLogin ? '<span class="indicator login" title="可登录">LOGIN</span>' : ''}
          </span>
        </div>
      </div>
      
      <div class="item-actions">
        <button class="btn btn-sm" data-action="copy" title="复制">复制</button>
        <button class="btn btn-sm" data-action="jump" title="查看">查看</button>
        <button class="btn btn-sm ${isFavorite ? 'active' : ''}" data-action="favorite" title="${isFavorite ? '取消收藏' : '收藏'}">${isFavorite ? '已收藏' : '收藏'}</button>
        <button class="btn btn-sm" data-action="forward" title="转发设置">转发</button>
        <button class="btn btn-sm" data-action="toggle-login" title="${canLogin ? '禁止登录' : '允许登录'}">${canLogin ? '禁用' : '允许'}</button>
        <button class="btn btn-sm danger" data-action="delete" title="删除">删除</button>
      </div>
    </div>
  `;
}
...
export function renderTableRow(mailbox) {
  ...
  return `
    <div class="table-row ${isPinned ? 'pinned' : ''}" data-address="${escapedAddress}">
      <div class="col-pin">
        <button class="btn btn-sm ${isPinned ? 'active' : ''}" data-action="pin">${isPinned ? 'PIN' : '置顶'}</button>
      </div>
      ...
      <div class="col-actions">
        <button class="btn btn-sm" data-action="copy" title="复制">复制</button>
        <button class="btn btn-sm" data-action="jump" title="查看">查看</button>
        <button class="btn btn-sm" data-action="more" title="更多">更多</button>
      </div>
    </div>
  `;
}
