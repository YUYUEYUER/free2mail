...
import { getJwtPayload, isStrictAdmin, sha256Hex, errorResponse, bumpApiCacheVersion } from './helpers.js';
...
export async function handleMailboxAdminApi(request, db, url, path, options) {
  ...
  if (path === '/api/mailboxes' && request.method === 'DELETE') {
    ...
    try {
      ...
      if (deleted) {
        ...
        bumpApiCacheVersion('mailboxes', 'mailboxInfo', 'emails', 'emailsBatch', 'quota', 'users', 'userMailboxes');
      }
  ...
  if (path === '/api/mailbox/forward' && request.method === 'POST') {
    ...
    const response = await handleSetForward(request, { TEMP_MAIL_DB: db });
    if (response?.ok) bumpApiCacheVersion('mailboxes', 'mailboxInfo');
    return response;
  }
  ...
  if (path === '/api/mailbox/favorite' && request.method === 'POST') {
    ...
    const response = await handleToggleFavorite(request, { TEMP_MAIL_DB: db });
    if (response?.ok) bumpApiCacheVersion('mailboxes', 'mailboxInfo');
    return response;
  }
  ...
  if (path === '/api/mailboxes/batch-favorite' && request.method === 'POST') {
    ...
    const response = await handleBatchFavorite(request, { TEMP_MAIL_DB: db });
    if (response?.ok) bumpApiCacheVersion('mailboxes', 'mailboxInfo');
    return response;
  }
  ...
  if (path === '/api/mailboxes/batch-forward' && request.method === 'POST') {
    ...
    if (!isStrictAdmin(request, options)) return errorResponse('Forbidden', 403);
    request.user = { role: 'strictAdmin' };
    const response = await handleBatchForward(request, { TEMP_MAIL_DB: db });
    if (response?.ok) bumpApiCacheVersion('mailboxes', 'mailboxInfo');
    return response;
  }
  ...
  if (path === '/api/mailboxes/batch-favorite-by-address' && request.method === 'POST') {
    ...
    if (!isStrictAdmin(request, options)) return errorResponse('Forbidden', 403);
    request.user = { role: 'strictAdmin' };
    const response = await handleBatchFavoriteByAddress(request, { TEMP_MAIL_DB: db });
    if (response?.ok) bumpApiCacheVersion('mailboxes', 'mailboxInfo');
    return response;
  }
  ...
  if (path === '/api/mailboxes/batch-forward-by-address' && request.method === 'POST') {
    ...
    if (!isStrictAdmin(request, options)) return errorResponse('Forbidden', 403);
    request.user = { role: 'strictAdmin' };
    const response = await handleBatchForwardByAddress(request, { TEMP_MAIL_DB: db });
    if (response?.ok) bumpApiCacheVersion('mailboxes', 'mailboxInfo');
    return response;
  }
