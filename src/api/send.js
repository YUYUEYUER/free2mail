...
import { getJwtPayload, errorResponse, getCachedJsonResponse, bumpApiCacheVersion } from './helpers.js';
...
export async function handleSendApi(request, db, url, path, options) {
  ...
  const SENT_LIST_TTL = 10 * 1000;
  const SENT_DETAIL_TTL = 10 * 1000;
  ...
  if (path === '/api/sent' && request.method === 'GET') {
    ...
    return getCachedJsonResponse(request, options, 'sentList', SENT_LIST_TTL, async () => {
      if (isMock) {
        return [];
      }
      ...
      keyParts: [String(from).trim().toLowerCase()]
    }).catch((e) => {
  ...
  if (request.method === 'GET' && path.startsWith('/api/sent/')) {
    ...
    return getCachedJsonResponse(request, options, 'sentDetail', SENT_DETAIL_TTL, async () => {
      if (isMock) {
      ...
      if (String(e?.message || '') === 'not-found') return errorResponse('未找到发件', 404);
  ...
  if (request.method === 'DELETE' && path.startsWith('/api/sent/')) {
    ...
    try {
      ...
      bumpApiCacheVersion('sentList', 'sentDetail');
