...
import { resolveAuthPayload, resolveAuthConfig } from '../middleware/auth.js';
...
export class AssetManager {
  ...
  getStaticAssetCacheControl(pathname) {
    if (/\.(?:js|css)$/i.test(pathname)) {
      return 'public, max-age=86400, stale-while-revalidate=604800';
    }

    if (/\.(?:svg|png|jpg|jpeg|gif|webp|avif|ico|woff2?|ttf|otf)$/i.test(pathname)) {
  ...
  async fetchStaticAsset(request, env, pathname) {
    const resp = await env.ASSETS.fetch(request);
    const cacheControl = this.getStaticAssetCacheControl(pathname);

    if (!cacheControl || !resp || !resp.ok) {
      return resp;
    }
  ...
  async handleAssetRequest(request, env, mailDomains) {
    ...
    const authConfig = resolveAuthConfig(env, request);
    const JWT_TOKEN = authConfig.jwtToken;
    ...
    if (pathname === '/' || pathname === '/index.html') {
      return await this.handleIndexPage(mappedRequest, env, mailDomains, authConfig);
    }

    if (pathname === '/admin.html') {
      return await this.handleAdminPage(mappedRequest, env, authConfig);
    }

    if (pathname === '/mailbox.html' || pathname === '/html/mailbox.html') {
      return await this.handleMailboxPage(mappedRequest, env, authConfig);
    }
    if (pathname === '/mailboxes.html' || pathname === '/html/mailboxes.html') {
      return await this.handleAllMailboxesPage(mappedRequest, env, authConfig);
    }

    return this.fetchStaticAsset(mappedRequest, env, pathname);
  }
  ...
  async handleIndexPage(request, env, mailDomains, authConfig) {
    const url = new URL(request.url);
    const payload = await resolveAuthPayload(request, authConfig.jwtToken);
  ...
  async handleAdminPage(request, env, authConfig) {
    const url = new URL(request.url);
    const payload = await resolveAuthPayload(request, authConfig.jwtToken);
  ...
  async handleMailboxPage(request, env, authConfig) {
    const url = new URL(request.url);
    const payload = await resolveAuthPayload(request, authConfig.jwtToken);
  ...
  async handleAllMailboxesPage(request, env, authConfig) {
    const url = new URL(request.url);
    const payload = await resolveAuthPayload(request, authConfig.jwtToken);
    if (!payload) {
      const loadingReq = new Request(
