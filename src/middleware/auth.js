...
/**
 * 是否为本地开发请求
 * @param {Request|string|URL} requestOrUrl
 * @returns {boolean}
 */
export function isLocalDevelopmentRequest(requestOrUrl) {
  try {
    const raw = typeof requestOrUrl === 'string'
      ? requestOrUrl
      : (requestOrUrl?.url || String(requestOrUrl || ''));
    const url = requestOrUrl instanceof URL ? requestOrUrl : new URL(raw || 'http://localhost/');
 ...
 * @param {object} env
 * @param {Request|string|URL} requestOrUrl
 * @returns {{isLocalDev:boolean,adminName:string,adminPassword:string,guestPassword:string,jwtToken:string,sessionExpireDays:number}}
 */
export function resolveAuthConfig(env = {}, requestOrUrl = '') {
  const isLocalDev = isLocalDevelopmentRequest(requestOrUrl);
  const adminName = String(env.ADMIN_NAME || 'admin').trim().toLowerCase();
  const adminPassword = String(env.ADMIN_PASSWORD || env.ADMIN_PASS || (isLocalDev ? 'admin' : '')).trim();
  const guestPassword = String(env.GUEST_PASSWORD || (isLocalDev ? 'admin' : '')).trim();
  const jwtToken = String(env.JWT_TOKEN || env.JWT_SECRET || (isLocalDev ? 'devtoken' : '')).trim();
  const sessionExpireDays = getSessionExpireSeconds(parseInt(env.SESSION_EXPIRE_DAYS, 10) || DEFAULT_SESSION_EXPIRE_DAYS) / (24 * 60 * 60);
...
export async function authMiddleware(context) {
  ...
  const root = checkRootAdminOverride(request, authConfig.jwtToken);
  if (root) {
    context.authPayload = root;
    return null;
  }

  const payload = await verifyJwtWithCache(authConfig.jwtToken, request.headers.get('Cookie') || '');
  if (!payload) {
