...
import { Router, createJwt, buildSessionCookie, verifyMailboxLogin, authMiddleware, resolveAuthConfig } from '../middleware/index.js';
...
export function createRouter() {
  ...
  router.post('/api/login', async (context) => {
    ...
    const ADMIN_NAME = authConfig.adminName;
    const ADMIN_PASSWORD = authConfig.adminPassword;
    const GUEST_PASSWORD = authConfig.guestPassword;
    const JWT_TOKEN = authConfig.jwtToken;
    const SESSION_EXPIRE_DAYS = authConfig.sessionExpireDays;
  ...
  router.get('/api/session', async (context) => {
    ...
    const ADMIN_NAME = resolveAuthConfig(env, request).adminName;
